...
def get_data():
    ...
    - Embarkation port mapped from 'S', 'Q', 'C' mapped to integers. Null mapped to mode.
    ...
    # Replace null fare value in test data with median for class in all data, and cap at 300.
    if len(test.Fare[test.Fare.isnull()]) > 0:
        ...
        train['Fare'] = train['Fare'].map(lambda y: min(y, 300.)).astype(float)
        test['Fare'] = test['Fare'].map(lambda y: min(y, 300.)).astype(float)
